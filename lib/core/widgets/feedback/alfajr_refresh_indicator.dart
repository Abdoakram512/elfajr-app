...
/// A custom, premium pull-to-refresh indicator tailored to Al-Fajr Charity Foundation's identity (مؤسسة الفجر).
/// Featuring a prominent animated glowing emblem, dynamic pull physics, and haptic feedback.
class AlfajrRefreshIndicator extends StatefulWidget {
  final Future<void> Function() onRefresh;
  final Widget child;
  final String? refreshingText;

  const AlfajrRefreshIndicator({
    super.key,
  ...
  @override
  State<AlfajrRefreshIndicator> createState() => _AlfajrRefreshIndicatorState();
}

class _AlfajrRefreshIndicatorState extends State<AlfajrRefreshIndicator>
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    ...
    if (notification is ScrollUpdateNotification) {
      ...
      if (notification.direction == ScrollDirection.idle) {
        if (_dragOffset >= _kTriggerThreshold) {
          _startRefresh();
  ...
  void _startRefresh() async {
    ...
    setState(() {
      _isRefreshing = true;
      _canRefresh = false;
      _dragOffset = _kTriggerThreshold;
    });
    ...
    try {
      ...
      // Handled upstream by Cubits/Views
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    final progress = (_dragOffset / _kTriggerThreshold).clamp(0.0, 1.0);
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: Stack(
        ...
        children: [
          // 1. Scaled / Shifted Child Viewport (smooth rubber-band physics)
          Transform.translate(
            offset: Offset(0, _dragOffset),
            child: widget.child,
          ),

          // 2. Custom Al-Fajr Pull-to-Refresh Header
          if (_dragOffset > 0)
            Positioned(
              top: 0,
              left: 0,
  ...
  Widget _buildHeaderContent(double progress) {
    return Column(
      ...
      children: [
        // Prominent Glowing Animated Al-Fajr Seal (Sun Dawn & Emerald Sprout Emblem)
        AnimatedBuilder(
...
typedef QoutRefreshIndicator = AlfajrRefreshIndicator;
